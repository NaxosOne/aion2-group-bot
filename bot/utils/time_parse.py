...
Error messages are localised: each parse function takes the server's language
and raises ParseError already carrying a translated, user-ready message.
"""
...
def _parse_time(s: str, lang: str = "en") -> tuple[int, int]:
    ...
    if not m or (m.group(3) is None and not re.search(r"[h:]", s)):
        raise ParseError(i18n.t("time.bad_time", lang))
    hour, minute, ampm = int(m.group(1)), int(m.group(2) or 0), m.group(3)
    if ampm:
        if not 1 <= hour <= 12:
            raise ParseError(i18n.t("time.invalid_time", lang, value=f"{hour}{ampm}"))
...
def parse_when(text: str, tz, now: datetime | None = None, lang: str = "en") -> datetime:
    ...
    if not s:
        raise ParseError(i18n.t("time.empty", lang))
    ...
    if date_part is not None:
        ...
        try:
            ...
            raise ParseError(
                i18n.t("time.invalid_date", lang, value=f"{day:02d}/{month:02d}")
            ) from None
        if dt <= now:
            if year is None:
                ...
                raise ParseError(i18n.t("time.date_past", lang))
        return dt
    ...
    if dt <= now:
        if offset is None:
            ...
            raise ParseError(i18n.t("time.time_past", lang))
    return dt
...
def parse_date(text: str, tz, now: datetime | None = None, lang: str = "en") -> date:
    ...
    if not m:
        raise ParseError(i18n.t("time.bad_date", lang))
    day, month = int(m.group(1)), int(m.group(2))
    year = m.group(3)
    if year is not None:
        year = int(year)
...
def parse_when_or_date(
    ...
    if s in DAY_KEYWORDS or _DATE_ONLY_RE.match(s):
        d = parse_date(s, tz, now, lang)
        return datetime(d.year, d.month, d.day, tzinfo=tz), False
    return parse_when(text, tz, now, lang), True
